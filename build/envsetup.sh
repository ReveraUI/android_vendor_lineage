...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.lineage.device | grep -q "$REVERA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $REVERA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.lineage.device | grep -q "$REVERA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.lineage.device | grep -q "$REVERA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
