...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.revera.device | grep -q "$REVERA_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.revera.device | grep -q "$REVERA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.revera.device | grep -q "$REVERA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
